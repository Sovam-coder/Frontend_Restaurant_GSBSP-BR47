...
// let filtered_repos_str = "";
let contri_data = [];
let url_params;

// Change according to duration of BuildWithLetsupgrade
let lu_start_date = "2020-06-01"; // 2020-03-01
let lu_end_date = "2020-08-31"; // 2020-05-31
...
async function getCommits(github_username, start_date, end_date) {
  ...
  filtered_repos_str = projects_list_filtered.list.map((repo) => "repo:" + repo).join(" ");
...
async function getPRs(github_username, start_date, end_date) {
  ...
  // filter merged PRs so that only the ones with the label 'lu20' are left
  prs_data_merged.items = prs_data_merged.items.filter(element => {
    if(element.labels.map(label => label.name.replace(/\s/g,'').toLowerCase()).includes('lu20')) {
      return true;
    } else {
      return false;
    }
...
async function loadProjectList() {
  const url_local = `./resource/project_list.json`;
  const response_projects_list = await fetch(url_local);
  projects_list = await response_projects_list.json();
...
if(url_params != undefined) {
  ...
  loadProjectListPromise.then(() => {
    let loadFilteredProjectListPromise = new Promise(async (resolve, reject) => {
      await getCommits(url_params.id, "2020-05-31", lu_end_date)
        .then(() => resolve())
        .catch(() => {
    ...
    loadFilteredProjectListPromise.then(async () => {
      await getIssues(url_params.id, lu_start_date, lu_end_date)
        .catch(() => {
      ...
      // performScoreCalc(url_params.id, lu_start_date, lu_end_date);
      await getPRs(url_params.id, lu_start_date, lu_end_date)
        .catch(() => {
      ...
      await getContributions(url_params.id, lu_start_date, lu_end_date);
    });
